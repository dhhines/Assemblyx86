...
getclp:                                         ; label for code to capture clp if exists
        ; cmp        byte ptr es:[80h], 0       ; compare the bytes in extra segment to 0
         ...
         mov        dh,[bx + valtbl]            ; set dh to the value at offset plus byte in bx
...
setspcs:                                        ; Label for set spaces routine
         ...
         mov       cl, [num_sp]                 ; Move the tab stop position into cl
         sub       cl, [count]                  ; subtract count from tab stop position for num spaces
         mov       dl, [asc_sp]                 ; move the ASCII value for space character into dl register
looptop:                                        ; Label for top of loop outputing spaces
         ...
         jmp       rstcntr                      ; Jump to get the next character from stdin
;------------------------------------------------
...
;------------------------------------------------
 output:                                        ; Output label for printing character
         ...
         cmp       dh,[count]                   ; Compare count to number spaces
         jb        rstcntr                      ; .... jump to reset counter if CF=1
