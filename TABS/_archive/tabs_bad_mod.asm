...
num_sp   dw        10                           ; Default number of spaces is 10
count    dw        0                            ; count of characters output before tab
...
getclp:                                         ; label for code to capture clp if exists
         ...
         mov        al, byte ptr es:[82h]       ; move the clp to the al register
         sub        al, [digdec]                ; convert from ASCII to decimal for digits
         mov        ah, 0                       ; set ah to 0 to complete word
         mov        [num_sp], ax                ; store the decimal value in [num_sp] variable
;------------------------------------------------
...
setspcs:                                        ; Label for set spaces routine
         mov       ax,[count]                   ; Move data word count into ax
         div       [num_sp]                     ; Unsigned divide of ax by number spaces
         mov       ch,0                         ; Set ch register to 0
         mov       cl,ah                        ; Get remainder of ax divide by number spaces
...
;------------------------------------------------
 output:                                        ; Output label for printing character
         ...
         cmp       al,[asc_lf]                  ; Compare to line feed character
         je        clrcntr                      ; .... jump to clear counter and then output
